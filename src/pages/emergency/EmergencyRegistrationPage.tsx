...
import { Patient, EmergencyTriageLevel, EmergencyCase } from '../../types';
import { 
  AlertOctagon, 
  Activity, 
  Search, 
...
import { EmergencyTreatmentModal } from './EmergencyTreatmentModal';

export const EmergencyRegistrationPage: React.FC = () => {
  const { patients, emergencyCases, addEmergencyCase } = useHospital();
  const navigate = useNavigate();

  // Multi-step state (1 to 5)
  const [currentStep, setCurrentStep] = useState<1 | 2 | 3 | 4 | 5>(1);

  // Step 1: Existing vs New vs Unknown
  ...
  // Completed case after submission
  const [submittedCase, setSubmittedCase] = useState<EmergencyCase | null>(null);
  ...
  const loadAbebeScenario = () => {
    ...
    setChiefComplaint('Severe chest pain radiating to left arm');
    setBpSystolic(90);
    setBpDiastolic(60);
    setPulse(115);
    setTemp(37.2);
  ...
  const loadUnknownTraumaPreset = () => {
    ...
    setFullName('Unknown Patient (Acute Trauma)');
    setAge(45);
    setGender('MALE');
    setPhone('Unidentified');
    setEmergencyContact('Unknown / Bystander arrival');
    setChiefComplaint('Severe acute trauma / Hemorrhagic shock');
  ...
  const resetForm = () => {
    ...
    setFullName('');
    setSearchQuery('');
    setPatientId(`PAT-00${Math.floor(150 + Math.random() * 50)}`);
    setVisitId(`ER-000${Math.floor(40 + Math.random() * 60)}`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">
                Emergency Patient Fast-Track Registration
              </h1>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Rapid clinical intake, nurse triage protocol, and immediate physician assignment without delaying care.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>Emergency Queue ({emergencyCases.length})</span>
          </Link>
          ...
          >
            ...
            <span>Preset: Abebe (Chest Pain)</span>
          </button>
        </div>
      </div>

      {/* Hospital Dual Registration Flow Banner */}
      <div className="bg-slate-900 text-white p-4 rounded-xl shadow-xs border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2.5">
          <GitBranch className="w-4 h-4 text-rose-400 shrink-0" />
          <div>
            <span className="font-bold text-rose-300">Hospital Registration Rule: </span>
            <span className="text-slate-300">
              Normal outpatients follow full front-desk registration. Emergency patients use this fast-track stream to ensure life-saving care starts within 0–2 minutes.
            </span>
          </div>
        </div>
        <button
          ...
          className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg border border-slate-700 font-medium transition text-xs whitespace-nowrap flex items-center gap-1"
        >
          <EyeOff className="w-3.5 h-3.5 text-slate-400" />
          Unknown Patient Preset
        </button>
      </div>
      ...
      {submittedCase ? (
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-emerald-200 shadow-sm space-y-6">
          <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-emerald-950">
                Emergency Registration Completed & Dispatched!
              </h3>
              <p className="text-xs text-emerald-700 mt-0.5">
                Visit ID <span className="font-mono font-bold">{submittedCase.id}</span> generated for {submittedCase.patientName}. Patient is active in the Emergency Queue.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs">
            <div>
              <span className="text-[11px] text-slate-500 uppercase font-semibold">Patient File</span>
              <p className="font-bold text-slate-900 mt-0.5">{submittedCase.patientName}</p>
              <p className="font-mono text-slate-500">{submittedCase.patientId}</p>
            </div>
            <div>
              <span className="text-[11px] text-slate-500 uppercase font-semibold">Acuity & Triage</span>
              <p className="font-bold text-rose-700 mt-0.5">🔴 {submittedCase.triageLevel}</p>
              <p className="text-slate-500">{submittedCase.chiefComplaint}</p>
            </div>
            <div>
              <span className="text-[11px] text-slate-500 uppercase font-semibold">Vital Signs</span>
              <p className="font-mono font-bold text-slate-900 mt-0.5">
                {submittedCase.vitalSigns?.bloodPressureSystolic}/{submittedCase.vitalSigns?.bloodPressureDiastolic} mmHg
              </p>
              <p className="font-mono text-slate-500">HR {submittedCase.vitalSigns?.pulseRate} | SpO2 {submittedCase.vitalSigns?.oxygenSaturation}%</p>
            </div>
            <div>
              <span className="text-[11px] text-slate-500 uppercase font-semibold">Doctor & Bay</span>
              <p className="font-bold text-slate-900 mt-0.5">{submittedCase.assignedDoctor}</p>
              <p className="text-slate-600">{submittedCase.bedAssigned}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-200">
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/emergency/dashboard"
                className="px-4 py-2.5 text-xs font-bold text-slate-800 bg-white hover:bg-slate-50 border border-slate-300 rounded-xl transition"
              >
                Go to Emergency Live Queue
              </Link>
              <button
                type="button"
              ...
              >
                ...
                <span>Treat Patient & Inpatient Admission (Step 7 & 8)</span>
              </button>
            </div>
          </div>
        </div>
              ...
              {[
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[11px] truncate max-w-full mt-0.5 hidden sm:inline">
                      {step.label}
                    </span>
                  </button>
                );
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-1">
                      Search by Patient ID (e.g. PAT-00125), Phone, or Name. Re-uses existing medical record.
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">New Patient (Quick Intake)</h4>
                    <p className="text-[11px] text-slate-500 mt-1">
              ...
              {patientType === 'EXISTING' && (
                <div className="p-4 border border-blue-200 bg-blue-50/30 rounded-xl space-y-3">
                  <label className="block text-xs font-bold text-slate-800">
                    Search Hospital Patient Directory (ID, Phone, or Name)
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Type 'Abebe', 'PAT-00125', or phone..."
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="px-5 py-2.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition flex items-center gap-1.5"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition flex items-center gap-1.5"
                >
                  Continue to Visit Details <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}

          {/* STEP 3: Emergency Visit Information */}
          {currentStep === 3 && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-rose-100 text-rose-700 text-xs font-bold flex items-center justify-center">
              ...
              <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Patient Intake</span>
                ...
                <div className="text-right">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Visit ID</span>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Severe crushing chest pain radiating to left arm..."
                  className="w-full px-3 py-2 text-xs border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-600 focus:outline-hidden"
                />
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition flex items-center gap-1.5"
                >
                  Proceed to Clinical Triage <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}

          {/* STEP 4: Triage & Immediate Vitals */}
          {currentStep === 4 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  ...
                  Clinical Triage Protocol & Vital Signs
                </h3>
                <span className="text-xs font-semibold text-slate-500">Nurse Triage Assessment</span>
              </div>

              {/* Priority Selector */}
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                <label className="block text-xs font-bold text-slate-800">
                  Triage Priority Level (Clinical Protocol) *
                </label>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 p-4 bg-white border border-slate-200 rounded-xl">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">e.g. 90/60</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 text-xs font-mono font-bold text-center border border-slate-300 rounded-lg focus:ring-2 focus:ring-rose-500"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">115 bpm (Tachycardia)</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 text-xs font-mono font-bold text-center border border-slate-300 rounded-lg focus:ring-2 focus:ring-rose-500"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">37.2°C</p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 text-xs font-mono font-bold text-center border border-slate-300 rounded-lg focus:ring-2 focus:ring-rose-500"
                  />
                  <p className="text-[10px] text-rose-600 font-bold mt-1">89% (Hypoxia alert)</p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-[10px] text-rose-600 font-bold mt-1">9/10 Severe</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition flex items-center gap-1.5"
                >
                  Continue to Physician Assignment <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          )}
          ...
          {currentStep === 5 && (
            <form onSubmit={handleFinalSubmit} className="space-y-4">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                ...
                Assign Physician, Bay & Complete Registration
              </h3>
              ...
              <div className="p-4 bg-rose-50/70 border border-rose-200 rounded-xl space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs pt-1">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-500 uppercase">SpO2 / Pain</span>
                    <p className="font-mono font-bold text-rose-700">{spo2}% | {painLevel}/10</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <span className="text-xs">⚡ IN PROGRESS (Immediate Hand-off)</span>
                  ...
                  >
                    ...
                    <p className="text-[11px] font-normal text-slate-500 mt-0.5">Queued on live ER board.</p>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                ...
                >
                  ...
                  Dispatch Emergency Case & Register
                </button>
              </div>
            </form>
          )}
      ...
      {/* Step 7 & 8 Doctor Treatment & Hospital Admission Modal */}
      {submittedCase && (
        <EmergencyTreatmentModal
          isOpen={isTreatmentModalOpen}
          onClose={() => setIsTreatmentModalOpen(false)}
          emergencyCase={submittedCase}
        />
      )}
    </div>
  );
